...
// 新增：選取需要淡入的容器
const wrapper = document.querySelector('.animation-scale-wrapper');
...
const textStages = [
    ...
    { threshold: 40, prefix: 'NOW LOADING', content: '"Multimodal Technology-Assisted Learning"', isChinese: false },
    { threshold: 60, prefix: 'NOW LOADING', content: '"Cognitive Load"', isChinese: false },
    { threshold: 80, prefix: 'NOW LOADING', content: '"Learning Interest"', isChinese: false },
    { threshold: 100, prefix: 'NOW LOADING', content: '"Self-Regulated Learning Habits"', isChinese: false }
];
...
function checkLoaded() {
    loadedCount++;
    // 當兩個圖層 (bg & fg) 都確實觸發 load 事件後
    if (loadedCount === 2) {
        setTimeout(startLoadingSequence, 100); // 微調延遲確保 DOM 渲染跟上
    }
}

// 1. 改聽 'load' 事件 (比 'ready' 更保證動畫檔案已完全載入)
bgPlayer.addEventListener('load', checkLoaded);
fgPlayer.addEventListener('load', checkLoaded);

// 2. 綁定完事件後，再透過 JavaScript 動態注入 src
// 這能 100% 避免快取造成的 Race Condition，保證一定會觸發 checkLoaded
const lottieUrl = "https://lottie.host/408a6eed-7fdc-4a8b-bff9-45fbfc9f0df6/s2RGeHlttm.lottie";
bgPlayer.setAttribute('src', lottieUrl);
fgPlayer.setAttribute('src', lottieUrl);
