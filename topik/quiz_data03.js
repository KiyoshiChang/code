/* =========================================
        [配置區] 您的 Google Form 設定 (修正版)
        =========================================
        */
        const GOOGLE_FORM_CONFIG = {
            // 修正處：這裡必須改成您新表單的 Action URL (注意 /d/e/ 的結構)
            formAction: "https://docs.google.com/forms/d/e/1FAIpQLScWi-VfJaf8d5Y6JOzStIxDXQlWm3FmbK1dE94vhrTmB2hHNw/formResponse", 
            
            // 這是您剛剛查到的正確欄位 ID
            entryId: "entry.1324922959",

            // 學生要填寫的問卷連結
            surveyUrl: "https://docs.google.com/forms/d/e/1FAIpQLScWi-VfJaf8d5Y6JOzStIxDXQlWm3FmbK1dE94vhrTmB2hHNw/viewform"
        };

        // Global variables
        let submitted = false; // 用於判斷 iframe 是否提交完成
        ...
        function loadPracticeQuestion(questionNum) {
            ...
            shuffledVocabs.forEach((vocab, index) => {
                html += `
                    <button class="btn-water btn-hover px-2 py-2 draggable flex items-center whitespace-nowrap" 
                        ...
                        <div class="audio-trigger-zone" onclick="event.stopPropagation(); playPracticeAudio('${vocab.audio}', this.closest('button'));">
            ...
            document.querySelectorAll('.draggable').forEach(btn => {
                btn.addEventListener('click', (e) => {
                   // No action on click to prevent conflicts, audio is handled by icon click
                });
            });
        ...
        function showQuestionResults() {
            ...
            const resultHeader = document.querySelector('#question-results h2');
            if (resultHeader) {
                resultHeader.textContent = `시험 결과 (점수: ${totalScore}점)`;
            }
            ...
            if (totalScore >= 80) {
                gradeMessage = `
                    <div class="text-center mb-2 flex flex-col gap-1 leading-snug">
                        <div class="text-3xl font-bold text-green-600">훌륭합니다!</div>
                        <div class="text-xl text-green-500">(Excellent!)</div>
                    </div>
                `;
                createConfetti();
                ...
                gradeMessage = `
                    <div class="text-center mb-2 flex flex-col gap-1 leading-snug">
                        <div class="text-3xl font-bold text-blue-600">더 열심히 하세요!</div>
                        <div class="text-xl text-blue-500">(Keep trying!)</div>
                    </div>
                `;
            }
            ...
            const practiceStatusHtml = `<div class="text-center mb-4 font-bold leading-snug">PRACTICE: ${practiceCorrectCount} / ${practiceAnsweredCount} / ${practiceTotalCount} 어휘</div>`;
            
            // [MODIFIED] Google Form Submission Button
            const googleBtnHtml = `
                <div class="flex flex-col items-center justify-center mb-2 gap-2">
                    <button onclick="sendToGoogleForm()" 
                       id="form-submit-btn"
                       class="btn-primary px-6 py-2 rounded-full shadow-md text-white font-bold hover:bg-blue-600 transition-all">
                       Submit Score & Open Survey
                    </button>
                    <span id="form-submit-status" class="text-sm font-bold text-blue-600"></span>
                </div>
            `;

            // 1. 設定基礎文字 (重複 3 次以確保夠長，中間用 &nbsp; 加一點小間隔)
            const baseText = "↑Click the button above to submit your score and fill out the survey.↑";
            const longText = `${baseText} &nbsp;&nbsp;&nbsp; ${baseText} &nbsp;&nbsp;&nbsp; ${baseText}`;

            // 2. 建立無縫結構：放入兩個 .marquee-content (雙胞胎)
            // 當第一個 .marquee-content 跑出去時，第二個剛好補位
            const marqueeHtml = `
                <div class="marquee-container">
                    <div class="marquee-wrapper">
                        <div class="marquee-content">${longText}</div>
                        <div class="marquee-content">${longText}</div>
                    </div>
                </div>
            `;
            ...
            document.getElementById('question-results-content').innerHTML = gradeMessage + practiceStatusHtml + googleBtnHtml + marqueeHtml + resultsHtml;
        ...
        // [NEW FUNCTION] Send Data to Google Form via hidden Iframe AND Redirect
// [方案 B] 直接將成績帶入問卷連結 (最穩定的做法)
function sendToGoogleForm() {
            const btn = document.getElementById('form-submit-btn');
            const statusSpan = document.getElementById('form-submit-status');
            
            btn.disabled = true;
            btn.classList.add('opacity-50');
            statusSpan.innerText = "Generating Survey...";
            
            // 1. 計算成績字串 (與之前相同)
            const correctCount = Object.values(questionAnswers).filter(a => a.correct).length;
            const totalScore = Math.round((correctCount / questionsData.length) * 100);
            
            let body = `TOPIK I Listening Test Result(後測)\n`;
            body += `Total Score：${totalScore} Points\n`;
            body += `Correct Answers：${correctCount} / ${questionsData.length}\n\n`;
            
            // ... (這裡保留原本產生詳細成績 body 的程式碼，省略以節省版面) ...
            // 如果您需要完整的 body 生成代碼，請保留原本 function 內的邏輯
            ...
            // 3. 關鍵修改：將資料編碼並拼接到 URL
            // 注意：這裡使用您提供的 entry ID: 1324922959 (不需要加 entry. 前綴，URL參數中只需要數字)
            // 但通常 Google Form URL 參數格式為 &entry.123456=Value
            
            const entryIdNumber = "1324922959"; // 您提供的 ID
            const encodedBody = encodeURIComponent(body);
            
            // 組合最終網址
            // 原本網址: .../viewform
            // 組合後: .../viewform?usp=pp_url&entry.1324922959=成績內容...
            
            const finalUrl = `${GOOGLE_FORM_CONFIG.surveyUrl}?usp=pp_url&entry.${entryIdNumber}=${encodedBody}`;
            
            // 4. 開啟問卷
            console.log("Opening URL:", finalUrl);
            window.open(finalUrl, '_blank');
            
            statusSpan.innerText = "Survey Opened!";
            setTimeout(() => {
                 btn.disabled = false;
        ...
        function sendResultToEmail() {
            ...
            const subject = 'TOPIK I Listening Test Result(後測)';
            
            const correctCount = Object.values(questionAnswers).filter(a => a.correct).length;
            const totalScore = Math.round((correctCount / questionsData.length) * 100);
            
            let body = `TOPIK I Listening Test Result(後測)\n Adapted by TOPIK I 기출문제/過去問題：\n\n`;
            body += `Total Score：${totalScore} Points\n`;
            body += `Correct Answers：${correctCount} / ${questionsData.length}\n\n`;
            ...
            try {
                ...
                // ▼▼▼ 修改這裡：使用 location.href 替代 window.open ▼▼▼
            ...
            fetch(question.vtt)
                ...
                .then(vttText => {
                    ...
                    firstHalfCues.forEach(cue => {
                        ...
                        if (lines[0]) {
                            subtitleHtml += `<div class="text-blue-600 font-bold leading-none my-0 mb-1">${lines[0]}</div>`;
                        }
                        if (lines[1]) {
                            subtitleHtml += `<div class="text-gray-600 leading-none my-0 mb-2">${lines[1]}</div>`;
                        }
                    });
